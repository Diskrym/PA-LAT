        ...
        while True:
            ...
            if inputs is not None : 
                ...
                if not missing_keys :
                    ...
                    print("prev altitude : ", previous_FlightParameter.altitude)
                    print("prev volets : ", previous_FlightParameter.flaps)
                    print("prev trains : ", previous_FlightParameter.landing_gear)
                    print("prev ap : ", previous_FlightParameter.autopilot)
                    print("altitude : ", current_FlightParameter.altitude)
                    print("volets : ", current_FlightParameter.flaps)
                    print("trains : ", current_FlightParameter.landing_gear)
                    print("ap : ", current_FlightParameter.autopilot)
                    print("flightphase before : ", perfo.flightPhase)
                    if (0 < current_FlightParameter.altitude < 41000):
                        perfo.setFlightPhase(previous_FlightParameter.altitude,current_FlightParameter.altitude)
                        print("flightphase after : ", perfo.flightPhase)
...
if __name__ == '__main__':
    ...
    takeoff=FlightPhase("takeoff",1,1,1,[0,13],decollage_vTable,decollage_vTargetsTable,decollage_nxTable,decollage_nzTable,decollage_fpaTargetsTable,decollage_roll_Table,decollage_roll_RateTable)
    initclimb=FlightPhase("initclimb",1,1,1,[14,5000],montée_initiale_vTable,montée_initiale_vTargetsTable,montée_initiale_nxTable,montée_initiale_nzTable,montée_initiale_fpaTable,montée_initiale_rollTable,montée_initiale_roll_RateTable)
    climbdesc=FlightPhase("climbdesc",0,0,0,[5001,24000],montée_descente_vTable,montée_descente_vTargetsTable,montée_descente_nxTable,montée_descente_nzTable,montée_descente_fpaTable,montée_descente_rollTable,montée_descente_rollrateTable)
    cruise=FlightPhase("cruise",0,0,0,[24001,41000],croisiere_vTable,croisiere_vTargetsTable,croisiere_nxTable,croisiere_nzTable,croisiere_fpaTable,croisiere_rollTable,croisiere_rollrateTable)
    approach=FlightPhase("approach",-1,1,1,[4999,0],approche_vTable,approche_vTargetsTable,approche_nxTable,approche_nzTable,approche_fpaTable,approche_rollTable,approche_roll_RateTable)
    ...
    IvyServerPerfo = Server("Perfo", bus_address, ["^IHM : (.*)", "^StateVector (.*)", "FCUVertical (.*)", "FCUAP1 (.*)"], regex_to_extract, perfo_interpreter)
